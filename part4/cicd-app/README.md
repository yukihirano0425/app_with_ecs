...
```bash
cicd-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
cicd-app$ sam local start-api
cicd-app$ curl http://localhost:3000/
```
...
```bash
cicd-app$ sam logs -n HelloWorldFunction --stack-name cicd-app --tail
```
...
```bash
cicd-app$ pip install pytest pytest-mock --user
cicd-app$ python -m pytest tests/ -v
```
